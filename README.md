...
are generated by `scripts/run_full_benchmark.py` against the real retrieval
pipeline and the real Self-RAG model, not synthetic unit tests, and persisted
at [`results/benchmark_results.md`](results/benchmark_results.md). Latest run:

**Retrieval quality (n=150)**

| Precision@1 | Recall@5 | MRR | Latency p50 / p95 |
|---|---|---|---|
| 64.7% | 91.3% | 0.780 | 2.9ms / 3.2ms |

**Guardrails audit (n=500 + 8 adversarial probes)**

| Query false-positive rate | Answer false-positive rate | Adversarial input catch | Adversarial output catch |
|---|---|---|---|
| 0.0% | 0.0% | 100.0% | 100.0% |

**Generation quality with the real Self-RAG model (n=30)** — oracle-passage single-shot vs. this repo's self-healing loop (real retrieval + retry):
...
The self-healing loop trades a 13.3% "I don't have enough information"
fallback rate for a 0% hallucination rate among the answers it does give —
the oracle baseline never refuses, but 1 in 5 of its answers is a
hallucination. Reproduce with `python -m scripts.run_full_benchmark
--sample-size 30`.
...
Contract text is indexed into a FAISS vector index per request, then
retrieved from and discarded. Two properties are load-bearing here and are
both covered by regression tests (`tests/test_retriever.py`):

- **Each request gets its own index, not a shared one.** An earlier version
  reused one global `LegalRetriever` across every request; since FAISS's
  `add()` is append-only and nothing ever cleared it, every contract ever
  analyzed since the process booted stayed searchable, so an unrelated
  earlier contract's content could bleed into an answer for a completely
  different request.
- **Isolation holds under concurrent requests, not just sequential ones.**
  Giving each request its own index but still mutating one shared retriever
  object wasn't sufficient -- the production server handles requests
  concurrently, so one in-flight request's indexing could still overwrite
  another's between that request's index step and its retrieve step. Each
  request now builds its own `LegalRetriever` (`_request_retriever()` in
  `backend/api.py`), sharing only the expensive-to-load embedding model.
...
`GET /health` returns the deployed commit (`RAILWAY_GIT_COMMIT_SHA`, auto-set
by Railway) alongside model/retriever load status — a quick way to confirm a
push actually rolled forward rather than the platform silently serving a
stale build.
...
retrieve → generate → critique ──accept──▶ return answer
                       ...
                       reject
                          │
              ┌───────────┴────────────┐
    passage irrelevant          passage relevant,
    (ISREL)                     answer unsupported (ISSUP)
              │                          │
              ▼                          ▼
   reformulate query → retrieve   resample generation on the
        (retry)                  SAME passage at higher temp
              │                          │
              └───────────┬──────────────┘
...
The critic rejects an answer when the passage is judged irrelevant to the
question (`ISREL`) or the answer isn't supported by it (`ISSUP`), or when
EigenScore flags high semantic divergence across resamples. These two
rejection reasons get healed differently: an irrelevant passage means
retrieval itself failed, so the query is reformulated (via the model itself,
or a rule-based fallback) and retrieval runs again. A relevant passage with
an unsupported answer means retrieval and generation already succeeded and
what's unreliable is the model's own single-sample critique judgment, so
instead of re-retrieving, generation is resampled on the same passage at a
higher temperature — confirmed against the real Self-RAG GGUF model that
ISSUP can be a deterministic false negative on a correct answer at
temperature 0, where reformulating the query does nothing. Either path
retries up to `max_attempts`. Exposed via `POST /analyze_contract_self_healing`.
